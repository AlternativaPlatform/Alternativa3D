		...
		alternativa3d function collectChildrenDraws(camera:Camera3D, lights:Vector.<Light3D>, lightsLength:int, useShadow:Boolean):void {
			...
			for (var child:Object3D = childrenList; child != null; child = child.next) {
				...
				if (child.visible) {
					...
					if (camera.hzEnabled) {
						culled = child.culling < 0 || (child.boundBox != null && camera.hzRenderer.checkBoundBox(child.boundBox, child.localToCameraTransform));
					} else {
						culled = child.culling < 0 || (child.boundBox != null && camera.occludersLength > 0 && child.boundBox.checkOcclusion(camera.occluders, camera.occludersLength, child.localToCameraTransform));
					}
					if (!culled) {
