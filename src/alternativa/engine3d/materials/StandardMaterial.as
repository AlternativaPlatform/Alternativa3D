...
package alternativa.engine3d.materials {
	...
	public class StandardMaterial extends TextureMaterial {
		...
		private static const END_OFFSET : int = 20;
		...
		private static function getPassTBNProcedure(right : Boolean) : Procedure {
			...
			"#v2=vNormal", //
			// Calculate binormal 
			crsInSpace, "mul t1.xyz, t1.xyz, i0.w", //
		...
		override alternativa3d function fillResources(resources : Dictionary, resourceType : Class) : void {
			...
			if (lightMap != null && A3DUtils.checkParent(getDefinitionByName(getQualifiedClassName(lightMap)) as Class, resourceType)) {
				resources[lightMap] = true;
			}
			if (glossinessMap != null && A3DUtils.checkParent(getDefinitionByName(getQualifiedClassName(glossinessMap)) as Class, resourceType)) {
				resources[glossinessMap] = true;
			}
			if (specularMap != null && A3DUtils.checkParent(getDefinitionByName(getQualifiedClassName(specularMap)) as Class, resourceType)) {
		 ...
		 * @private
		 */
		alternativa3d function setVertexBuffers(destination : DrawUnit, geometry : Geometry, vertexLinker : Linker) : void {
		}
		...
		private function formDirectionalProcedure(procedure : Procedure, index : int, useShadow : Boolean) : void {
			var source : Array = [// Position - dirction vector of light
			"#c0=c" + index + "Position", "#c1=c" + index + "Color",
			// Calculate half-way vector 
			"add t0.xyz, i1.xyz, c0.xyz", "mov t0.w, c0.w", "nrm t0.xyz,t0.xyz",
			// Calculate a flare 
			"dp3 t0.w, t0.xyz, i0.xyz", "pow t0.w, t0.w, o1.w",
			// Calculate light 
			"dp3 t0.x, i0.xyz, c0.xyz", "sat t0.x, t0.x"];
			if (useShadow) {
		...
		private function formOmniProcedure(procedure : Procedure, index : int, useShadow : Boolean) : void {
			...
			var source : Array = ["#c0=c" + index + "Position", "#c1=c" + index + "Color", "#c2=c" + index + "Radius", "#v0=vPosition"];
			if (useShadow) {
				// Считаем вектор из точки к свету
				source.push("sub t0, c0, v0");
				// L = lightPos - PointPos
				source.push("dp3 t0.w, t0.xyz, t0.xyz");
				// lenSqr
				source.push("nrm t0.xyz, t0.xyz");
				// L = normalize(L)
				...
				source.push("sqt t1.x, t0.w");
				// len = sqt(lensqr)
				// Считаем свет
				source.push("dp3 t0.w, t0.xyz, i0.xyz");
				// dot = dot(normal, L)
				// Считаем затухание
				source.push("sub t0.x, t1.x, c2.z");
				// len = len - atenuationBegin
				source.push("div t0.y, t0.x, c2.y");
				// att = len/radius
				source.push("sub t0.x, c2.x, t0.y");
				// att = 1 - len/radius
				source.push("sat t0.xw, t0.xw");
				// t = max(t, 0)
				...
				if (lightsLength > 0 || shadowedLight) {
					...
					for (i = 0; i < lightsLength; i++) {
						...
						if (light is DirectionalLight) {
							...
							lightFragmentProcedure.compileFromArray(["#c0=c" + i + "Position", "#c1=c" + i + "Color", "#c2=c" + i + "Radius", "#c3=c" + i + "Axis", "#v0=vPosition",
							// Calculate vector from the point to light 
							"sub t0, c0, v0",// L = pos - lightPos 
							"dp3 t0.w, t0, t0",// lenSqr 
							"nrm t0.xyz,t0.xyz",// L = normalize(L)
							// Calculate half-way vector 
							"add t2.xyz, i1.xyz, t0.xyz", "nrm t2.xyz, t2.xyz",
							// Calculate a flare 
							"dp3 t2.x, t2.xyz, i0.xyz", "pow t2.x, t2.x, o1.w", "dp3 t1.x, t0.xyz, c3.xyz", // axisDirDot 
							...
							"sub t0.y, t1.x, c2.w", "div t0.y, t0.y, c2.z", "sat t0.xyw,t0.xyw",// t = sat(t) 
		...
		private function addDrawUnits(program : StandardMaterialProgram, camera : Camera3D, surface : Surface, geometry : Geometry, opacityMap : TextureResource, lights : Vector.<Light3D>, lightsLength : int, isFirstGroup : Boolean, shadowedLight : Light3D, opaqueOption : Boolean, transparentOption : Boolean, objectRenderPriority : int) : void {
			...
			setVertexBuffers(drawUnit, geometry, program.vertexShader);
			// Constants
		...
		override alternativa3d function collectDraws(camera : Camera3D, surface : Surface, geometry : Geometry, lights : Vector.<Light3D>, lightsLength : int, useShadow : Boolean, objectRenderPriority : int = -1) : void {
			...
			if (groupsCount == 0 && shadowGroupLength == 0) {
				...
				materialKey |= getUniqueMaterialKey(END_OFFSET);
		...
		alternativa3d function getUniqueMaterialKey(startOffset : int) : int {
			startOffset;
			return 0;
		}
